...
const COLORS = { bg: '#0A0F1E', surface: '#111827', border: '#1E293B', primary: '#22D3EE', text: '#F1F5F9', muted: '#64748B' };
...
export default function RegisterScreen({ navigation }: any) {
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      if (data.success) await login(data.data);
    } catch (e: any) {
  ...
  return (
    <View style={styles.container}>
      <View style={styles.card}>
        ...
        <TextInput style={styles.input} placeholder="Password (min 8 chars)" placeholderTextColor={COLORS.muted} value={password} onChangeText={setPassword} secureTextEntry />
        <TouchableOpacity style={[styles.button, loading && { opacity: 0.5 }]} onPress={handleRegister} disabled={loading}>
